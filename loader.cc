...
static int load_binary_bfd( std::string &fname, Binary *bin, Binary::BinaryType type){
  int ret;
  bfd *bfd_h;
  conts bfd_arch_info_type *bfd_info;
  ...
  switch(bfd_h->xvec->flavour){
    ...
    case bfd_target_coff_flavour:
      bin->flavour = Binary::BIN_TYPE_PE;
      break;
  ...
  switch(bfd_info->mach){
    ...
    case bfd_mach_x86_64:
      ...
      break;
    default:
      fprintf(stderr,"unsupported architecture (%s)\n", bfd_h->printable_name);
      goto fail;
  }
  /*Symbol handling, attention that they may not even be present..*/
...
static int load_symbols_bfd(bfd *bfd_h, Binary bin){
  ...
  if(n<0){
    ...
    if(nsyms < 0){
      fprintf(stderr, "Fauled to read symtab (%s)\n", bfd_errmsg(bfd_get_error()));
      goto fail;
    }
    for(i=0; i<nsyms;i++){
